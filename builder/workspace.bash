...
function list_packages_overlayers {
    local src=$1
    shift
    local overlayer_wss="$*"
    local cmd=("/opt/ros/$ROS_DISTRO/share")
    if [[ -n "${overlayer_wss[@]}" ]]; then
        for ws in "${overlayer_wss[@]}"; do
            cmd+=("$ws/install/*")
        done
    fi
    cmd+=("$src")
    ...
    if [ "$ROS_VERSION" -eq 1 ]; then
        "/opt/ros/$ROS_DISTRO/env.sh" catkin_topological_order --only-names ${cmd[@]}
    fi
...
function resolve_depends_overlayers {
    local src=$1
    shift
    local args="$*"
    shift
    ...
    if [[ "$ROS_VERSION" -eq 1 ]]; then
        comm -23 <(read_depends "$src" "${deps[@]}" | sort -u) <(list_packages_overlayers "${overlayers[@]}" | sort -u) | xargs -r "/opt/ros/$ROS_DISTRO"/env.sh rosdep resolve | grep_opt -v '^#' | sort -u
    fi
    if [[ "$ROS_VERSION" -eq 2 ]]; then
        comm -23 <(read_depends "$src" "${deps[@]}" | sort -u) <(list_packages_overlayers "${overlayers[@]}" | sort -u) | xargs -r rosdep resolve | grep_opt -v '^#' | sort -u
    fi
    if [ "$ROS_VERSION" -ne 2 ] && [ "$ROS_VERSION" -ne 1 ]; then
        echo "Cannot get ROS_VERSION"
        exit 1
...
function test_workspace {
    ...
    if [[ -n "${overlayers[@]}" ]]; then
        for overlayer in "${overlayers[@]}"; do
            source "$overlayer/install/local_setup.bash"
            echo "source $overlayer/install/local_setup.bash"
        done
    fi

    resolve_depends_overlayers "$ws/src" --overlayers "${overlayers[@]}" --deps depend exec_depend run_depend test_depend | apt_get_install
